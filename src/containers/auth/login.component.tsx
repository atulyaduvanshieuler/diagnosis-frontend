...
const Login = () => {
  ...
  const handlePasswordInput = (event: any) => {
    if (event.target.value.length < 4) {
      ...
      updateFormError({
        message: "Please enter password (min 4 characters)",
        isError: true,
      });
    } else {
      updatePassword(event.target.value);
  ...
  const handleLogin = async () => {
    if (email && password) {
      ...
      const response: any = await postRequest(API.LOGIN, {
        user_key: email,
        password: password,
      });
    ...
    <div className={"login-container"}>
      <div className={"left-section"}>
        ...
        <div className={"left-section-content"}>
          ...
          <div className={"left-section-subheading"}>
      ...
      <div className={"form-container"}>
        <form className={"form"} autoComplete={"on"}>
          ...
          />
          {formError?.isError ? (
            <div className={"error-text"}>{formError.message}</div>
          ) : (
            ""
          )}
